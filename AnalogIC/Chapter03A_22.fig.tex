\documentclass{xStandalone}

\begin{document}
\begin{tikzpicture}

\xSetX
\xValDefine{xL}[-5]
\xValDefine{xR}[+5]
\foreach \x/\s in {0/A,1/B,2/C,3/D,4/E,5/F}
{
    \xValPropor{xP\s}{xL}{xR}[\x/5]
    \ifthenelse{\x=5}{}{\xValOffpro{xQ\s}{xP\s}{xL}{xR}[1/10]}
}

\xSetY
\xValDefine{yB}[-2]
\xValDefine{yA}[+2]
\xValMiddle{yC}{yA}{yB}

\draw (xL|-yA) node[left] {G} to[short,o-o] (xQA|-yA);
\draw (xL|-yB) node[left] {B} to[short,o-o] (xR|-yB) node[right] {B};
\draw (xQA|-yC) node[left] {S} to[short,o-o] (xQE|-yC) node[right] {S};
\draw (xR|-yA) node[right] {D} to[short,o-] (xPB|-yA);

\draw (xPB|-yB) node[ground] {};

\draw (xPB|-yA) to[cisource,l=$(g_{m1}+g_{m2})V_{GS}$,-*] (xPB|-yC);
\draw (xPD|-yA) to[R,l=$r_{O1}$,*-*]  (xPD|-yC);
\draw (xPE|-yA) to[R,l_=$r_{O2}$,*-*] (xPE|-yC);
\draw (xPB|-yC) to[short,-*] (xPB|-yB);


\xDrawVoltage{B}(xPA|-yA)(xPA|-yB)[$V_{in}$]
\xDrawVoltage{B}(xPF|-yA)(xPF|-yB)[$~~~V_{out}$]
\xDrawVoltage{B}(xQA|-yA)(xQA|-yC)[$V_{GS}$]
\xDrawVoltage{A}(xQA|-yB)(xQA|-yC)[$V_{BS}$]
\xDrawVoltage{B}(xQE|-yA)(xQE|-yC)[$V_{DS}$]

\xValBorder{xL}[-0.75]{xR}[+0.75]{yB}[-0.8]{yA}*


\end{tikzpicture}
\end{document}