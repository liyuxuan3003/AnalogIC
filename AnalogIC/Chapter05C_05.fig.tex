...
\draw (xC|-yPD) node[above] {$V_P$} to[open,*-] (xC|-yPE) node[ground] {} node[ocirc] {};
...
\draw (M1.G) to[short] (xL|-M1.G) node[ground] {};
\draw (M2.G) to[short] (xR|-M2.G) node[ground] {};

\draw (xLx|-yPC) node[circ] {} node[left] {$V_F$};

\draw (xRx|-yPC) to[short,*-o,i<=$I_{out}$] (xR|-yPC) node[right] {$V_{out}$};
