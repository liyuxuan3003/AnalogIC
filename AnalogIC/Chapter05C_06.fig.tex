...
\draw (xC|-yPD) node[above] {$V_P$} to[R={$R_{SS}=r_{O5}$},*-] (xC|-yPE) node[ground] {};
...
\draw (M1.G) to[short,-o] (xL|-M1.G) node[below] {$V_{in,CM}$};
\draw (M2.G) to[short,-o] (xR|-M2.G) node[below] {$V_{in,CM}$};

\draw (xLx|-yPC) node[circ] {} node[left] {$V_F$} -- (xRx|-yPC);
