...
计算等效转移电导，对于\xref{fig:共源放大的诺顿等效电路}，将输出端口短接，如\xref{fig:计算源极负反馈的共源放大等效转移电导}所示
...
\end{Equation}
我们可以关注一下\xrefpeq{1}和\xrefpeq[计算等效转移电导共源]{1}的联系和差异，它们对应了\xref{fig:计算源极负反馈的共源放大等效输出电导}和\xref{fig:计算源极负反馈的共源放大等效转移电导}。
...
对比\xref{fml:源极反馈的共源放大等效转移电导}和\xref{fml:源极反馈的共源放大等效输出电导}，若改记$r_{O}^{-1}=g_O$
\begin{Equation}
...
这里先来计算$R_{out}$，应用\fancyref{fml:源极反馈的共源放大等效输出电导}
\begin{Equation}&[3]
...
\end{Equation}
而\fancyref{fml:源极反馈的共源放大等效转移电导}又告诉我们
\begin{Equation}&[5]
