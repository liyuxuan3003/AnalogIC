...
\draw (xQA|-yC) node[left] {S} to[short,o-o] (xPD|-yC) node[right] {S};
\draw (xR|-yA) node[right] {D} to[short,o-] (xPB|-yA);

\draw (xPB|-yB) node[ground] {};

\draw (xPB|-yA) to[cisource,l=$g_{m1}V_{GS}$,-*] (xPB|-yC);
\draw (xQD|-yA) to[R=$(g_{m2}+g_{mb2})^{-1}$,*-*] (xQD|-yB);
...
\xDrawVoltage{B}(xPD|-yA)(xPD|-yC)[$V_{DS}$]
