...
现在我们计算增益，可以证明
\begin{Equation}
    A_{CM}=\pdv{V_{out}}{V_{in}}=\frac{1}{2}\mu_nC_{ox}(W/L)\frac{\qty[4I_{SS}/\mu_nC_{ox}(W/L)]-2V_{in}^2}{\sqrt{\qty[4I_{SS}/\mu_nC_{ox}(W/L)]-V_{in}^2}}
\end{Equation}
我们特别关心的是$V_{in}=0$时的增益，此时差分放大器位于平衡点，增益最大
\begin{Equation}
    A_{CM}=\frac{1}{2}\mu_nC_{ox}(W/L)\sqrt{4I_{SS}/\mu_nC_{ox}(W/L)}
\end{Equation}
化简得到
\begin{Equation}
    A_{CM}=\sqrt{\mu_nC_{ox}(W/L)I_{SS}}
\end{Equation}
整理结论如下
