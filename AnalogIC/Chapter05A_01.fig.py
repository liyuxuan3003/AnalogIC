...
netlist1=netlist
netlist1.set_element_model("M1","xNMOS")
netlist1.set_element_model("M2","xNMOS")
...
netlist2=netlist
netlist2.set_element_model("M1","xNMOSLO")
netlist2.set_element_model("M2","xNMOSLO")
netlist2.add_instruction(r".dc Vout 0 4 0.001")
raw,log=runner.run_now(netlist1,run_filename="Temp.net")
data=RawRead(raw)
xIoutLO=data.get_trace("Id(M2)").get_wave(0)

plt.figure(idIout)
plt.plot(xVout,xIoutLO,c='gray',ls='dashed',lw=0.7,label=r'$I_{out}, \lambda=0$')
plt.plot(xVout,xIout,c='black',label=r'$I_{out}, \lambda\neq 0$')
plt.plot(xVout[[p2]],xIout[[p2]],marker='o',markersize=4,c='w',mec='k',ls='')
